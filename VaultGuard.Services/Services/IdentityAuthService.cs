...
public class IdentityAuthService : IAuthService
{
    ...
    public async Task<string?> GetCurrentUserIdAsync()
    {
        try
        {
            ...
            // If authenticated but no specific user ID, get the first active user (for backward compatibility)
            if (_isAuthenticated)
            {
                var user = await _dbContext.Users.Where(u => u.IsActive).FirstOrDefaultAsync();
                return user?.Id;
            }
