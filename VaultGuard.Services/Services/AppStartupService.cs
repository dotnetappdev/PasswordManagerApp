...
public class AppStartupService : IAppStartupService
{
    ...
    /// </summary>
    private Task EnsureVaultSchemaAsync(VaultGuardDbContext dbContext)
        // Delegates to the shared SqliteSchemaGuard so WPF, MAUI and the Blazor Web app all apply the
        // exact same additive SQLite fixes (single source of truth).
        => SqliteSchemaGuard.EnsureVaultSchemaAsync(dbContext, _logger);
