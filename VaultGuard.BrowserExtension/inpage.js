...
(function () {
  ...
  // Whether Vault Guard acts as the virtual WebAuthn authenticator for this page — i.e. whether it
  // intercepts passkey creation/use so keys are generated by and stored IN the vault instead of by
  // the platform (Windows Hello / Touch ID). Controlled by the user's "Intercept website passkeys"
  // setting: the content script passes the initial value via the injected <script>'s data-attribute
  // and pushes live changes with a CONFIG message. When OFF we transparently defer to the browser's
  // real authenticator, so nothing changes for the user.
  let intercepting = (function () {
    try {
      const el = document.currentScript;
      return !el || el.dataset.pmIntercept !== '0';
    } catch (_) { return true; }
  })();
  ...
  window.addEventListener('message', (event) => {
    ...
    // Live enable/disable of interception from the popup toggle (no page reload needed to turn OFF).
    if (event.data.type === CONFIG) { intercepting = !!event.data.intercept; return; }
  ...
  navigator.credentials.create = async function (options) {
    try {
      // Interception disabled by the user, or a non-passkey request → use the platform authenticator.
      if (!intercepting || !options || !options.publicKey) {
